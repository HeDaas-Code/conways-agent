...
Before any new worldview fragment is written to disk, check it against
the existing worldview for contradictions using LLM-powered semantic analysis.
"""
...
class Conflict:
    """
    A detected contradiction between new and existing content.

    Attributes:
        existing_fragment: Title of the existing fragment
        existing_claim: The contradicting claim from existing worldview
        new_claim: The new contradicting claim
        severity: "hard" (definite contradiction) or "soft" (potential tension)
...
class ConsistencyCheck:
    """
    Result of checking a new fragment against existing worldview.

    Attributes:
        is_consistent: Whether the fragment is consistent with existing worldview
        conflicts: List of detected contradictions
        warnings: Non-blocking warnings (e.g., LLM failure, empty worldview)
...
class ConflictResolution:
    ...
    Attributes:
        ...
        adjusted_fragment: The fragment after resolving conflicts
...
class ConsistencyEngine:
    """
    Checks new worldview fragments against existing content for contradictions.
    ...
    def check(self, fragment: WorldFragment) -> ConsistencyCheck:
        """
        Check a new fragment against existing worldview for contradictions.
        ...
        """
        try:
            existing = self._get_relevant_fragments(fragment)

            if not existing:
                return ConsistencyCheck(
                    is_consistent=True,
                    warnings=["No existing fragments to check against"],
            ...
            return ConsistencyCheck(
                is_consistent=True,
                warnings=[f"LLM check failed: {e}, proceeding conservatively"],
                checked_at=datetime.now()
            )
...
```"""
            ...
            response = self.llm.complete_str(system_prompt, user_prompt, temperature=0.3)

            log_event(
                "consistency_analysis",
                f"Conflict analysis for '{fragment.title}': {response[:200]}...",
                {
    ...
    def resolve_conflict(
        ...
        try:
            hard_conflicts = [c for c in check.conflicts if c.severity == "hard"]
            soft_conflicts = [c for c in check.conflicts if c.severity == "soft"]
    ...
    def _resolve_hard_conflicts(
        ...
        conflict_summary = self._format_conflicts(hard_conflicts + soft_conflicts)
...
```"""
        ...
        response = self.llm.complete_str(system_prompt, user_prompt, temperature=0.7)
    ...
    def _resolve_soft_conflicts(
        ...
        conflict_summary = self._format_conflicts(soft_conflicts)
...
```"""
        ...
        response = self.llm.complete_str(system_prompt, user_prompt, temperature=0.6)
    ...
    def _get_relevant_fragments(self, fragment: WorldFragment) -> list[WorldFragment]:
        """
        Get relevant existing fragments using keyword filtering.

        Args:
            fragment: The new fragment to find relevant matches for
        ...
        try:
            ...
            for md_file in sorted(world_dir.glob("*.md")):
                ...
                if self._is_relevant(content, keywords):
                    title = self._extract_title_from_markdown(content) or md_file.stem
                    relevant.append(WorldFragment(
                        title=title,
                        content=content,
            ...
            log_event(
                "consistency_fragment_read_error",
                f"Failed to read existing fragments: {e}",
                {"error": str(e)}
            )
            return []

    def _extract_keywords(self, fragment: WorldFragment) -> set[str]:
        """Extract meaningful keywords from a fragment."""
        text = f"{fragment.title} {fragment.content}".lower()
        ...
        stopwords = {
            ...
            '什么', '怎么', '如果', '因为', '所以', '虽然', '但是', '而且'
        }

        keywords = {w for w in words if len(w) >= 2 and w not in stopwords}
        return keywords

    def _is_relevant(self, content: str, keywords: set[str]) -> bool:
        """Check if content is relevant based on keyword overlap."""
        content_lower = content.lower()
        content_keywords = self._extract_keywords(
            WorldFragment(title="", content=content)
        )

        if not keywords or not content_keywords:
            return True

        overlap = keywords & content_keywords
        return len(overlap) >= min(2, len(keywords) // 2)
...
{frag.content[:500]}{'...' if len(frag.content) > 500 else ''}""")
    ...
    def _format_conflicts(self, conflicts: list[Conflict]) -> str:
        ...
        for i, conflict in enumerate(conflicts, 1):
            formatted.append(f"""**矛盾 {i}** ({conflict.severity})
        ...
        try:
            ...
            for item in data.get("conflicts", []):
                ...
                matching_fragment = next(
                    (f for f in existing if f.title == existing_title),
            ...
            log_event(
                "consistency_parse_error",
                f"Failed to parse conflicts from LLM response: {e}",
